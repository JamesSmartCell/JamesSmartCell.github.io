...
contract AttestationMintable is Context, ERC165, IERC721, IERC721Metadata {
    ...
    //string  constant _baseURI = "https://ipfs.io/ipfs/Qmcob1MaPTXUZt5MztHEgsYhrf7R6G7wV8hpcweL8nEfgU/meka/"; //Meka
    string constant _baseURI = "https://ipfs.io/ipfs/QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/";         //Punks
    ...
    modifier onlyOwner() {
        require(_contractCreator == _msgSender(), "Message Token: caller is not the owner");
        _;
    }
     ...
     */
    constructor () {
        _name = "Message Writer Token";
        _symbol = "MSG";
        _contractCreator = payable(msg.sender);

        //Minimal code to ensure wallets/opensea pick up 10 tokens (need to emit the event for each mint)
        _balances[_contractCreator] = 10;

        //first 10 tokens to creator
        for (uint tokenId = 1; tokenId <= 10; tokenId++)
        {
            _owners[tokenId] = _contractCreator;
            emit Transfer(address(0), _contractCreator, tokenId);
        }
    ...
    function mintUsingSequentialTokenId() public returns (uint256 tokenId) {
        //must have a token to mint
        require(_contractCreator == msg.sender || _balances[msg.sender] > 0, "Need to own a token to mint");
